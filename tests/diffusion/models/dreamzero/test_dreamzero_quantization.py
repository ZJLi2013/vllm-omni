...
`--quantization fp8` used to be a no-op here: none of the block linears received
a `quant_config`, so every GEMM silently stayed bf16. These tests pin the two
things that can regress without any visible error:

1. every GEMM-carrying linear gets the `quant_config`, and nothing else does;
2. each linear's quantization `prefix` equals its module path. The FFN is an
   `nn.Sequential`, whose children are named "0"/"2" — a hand-written prefix
   that disagrees would make `ignored_layers` and the loaded scales miss the
   layer while still loading "successfully".

CPU-only: the parallel linears and the attention layer are replaced by stand-ins,
following tests/diffusion/models/minimax_h3/test_minimax_h3_quantization.py.
"""
...
class _FakeLinear(nn.Module):
    """Stand-in recording `quant_config`/`prefix`, plus what init_weights touches."""
...
def test_every_gemm_linear_gets_the_quant_config(dz, num_layers):
    """The count is the whole point: a partial wiring loads fine and stays bf16."""
...
def test_default_is_unquantized_and_prefixes_still_set(dz):
    """Without a quant_config nothing changes, but the prefixes must survive."""
...
def test_non_gemm_modules_stay_full_precision(dz):
    """Embeddings, head and the action encoder are plain torch — never quantized."""
